...
ou vetores/matrizes, e podem retornar apenas valores primitivos. Entretanto, não
é permitido declarar funções aninhadas ou funções com parâmetros variáveis. Vale
ressaltar que passagem de parâmetros é sempre feita por \textit{valor}.
...
``olamundo'' (ou ``olamundo.exe'') no diretório atual. Após executar o
programa criado, é exibido o texto ``Ola mundo!'' na tela.
...
exibido:
  ...
  A-Z) seguido de qualquer número de letras ou  números (e underlines).
...
índice 0 e seus indices são sempre inteiros positivos. Logo, a matriz ``bitset''
pode ser usada do indice 0 até o indice 9 (inclusive). Seu índice 10 não é
...
simples,...), variáveis primitivas ou indices de vetores/matrizes. Os
...
termo ``2'' é promovido para o tipo ``real'', tendo o valor ``2.0'' antes
 que  a soma seja processada. O segundo caso é discutido na
...
é absoluto (\textit{overloaded}, depende do tipo de \textit{lvalue}) e, no mome
nto, é restrita expressões isentas de operadores. Isso é, não é permitido
aplicá-la como um termo em uma expressão com multiplos termos (ex. ``x :=  y +
leia()``), embora seja possível utilizá-la em subscritos de vetores e matrizes,
assim como em expressões de estruturas de controle. 
...
Na versão atual é possível traduzir algoritmos para a linguagem C. O resultado
é um código em C 100\% equivalente ao algoritmo.

Atualmente, a compilação é possível apenas usando o compilador GCC (uma vez que
GPT o usa como \textit{backend}) mas, futuramente, a compilação será feita sem
dependências externas.

A execução de algorítmos é feita de forma interpretada, isso é, sem gerar
arquivos binários. Por meio da interpretação, é possível depurar passo a passo
o algorítmo com o uso de uma ferramenta auxiliar (um cliente de depuração).

Vale ressaltar que, qualquer que seja o modo de uso do algoritmo
(compilado, interpretado ou traduzido), eles terão sempre comportamentos
equivalentes. Isso é, o resultado de uma execução interpretada deve ser o mesmo
se a execução fosse feita por meio de um binário compilado.
...
\begin{itemize}
 \item 
  ...
  \item
  d: Exibe maiores informações para erros de compilação, quando disponível.
Essas informações são dicas de como proceder para resolver o erro;
...
\textit{<arquivo>};
  ...
  \item
  t: Ao traduzir um algoritmo para C, salva o arquivo fonte como
\textit{<arquivo>.c};

  \item 
  i: Executa o algoritmo diretamente, sem compilar ou criar arquivos binários
executáveis. Opção conhecida como ``interpretação'' ou ``scripting''.

  \item
  H: Usado em conjunto com a opção ``-P'', para se conectar a um
\textit{client debugger};

  \item
  P: Usado em conjunto com a opção ``-P'' para se conectar a um \textit{client
debugger}.
\end{itemize}


%  *mudanca de linguagem (sub linguagens, variacoes de gramatica) 
...
local exato onde o erro foi localizado (linha e coluna no código fonte).
Deve, também, fornecer dicas de como proceder para corrigir o erro, em
alguns casos.
...
\section{Geração e Execução de programas}
...
compilador pode emitir erros, visto que \textit{printf} é uma função da
...
(como ``\underline{ }\underline{ }leia\underline{ }texto'') devem ser evitados,
pois muitos identificadores
internos utilizam essas convenções de nomeação, e seu uso pode acarretar em
conflitos durante a tradução.

\subsection{Interpretação de código}

GPT permite que o algoritmo seja executado sem gerar código binário. Esse modo
é conhecido como ``interpretação'' e linguagens como Perl, PHP e Ruby utilizam
esta técnica.
...
inspeção de variáveis locais/globais e pontos de parada
(``\textit{breakpoints}'').
...
informações enviadas pelo GPT (variáveis, breakpoints, etc) usam o formato XML,
enquanto o cliente envia comandos simples (o que evita que o programa GPT
nescessite de um XML parser).
...
A seguir é apresentado as convenções léxicas usadas em G-Portugol, usando
notações de expressões regulares.

\begin{verbatim} 
    Regras para identificar literais numéricos

T_INT_LIT : T_OCTAL_LIT | T_HEX_LIT | T_BIN_LIT | [0-9]+
...
A seguir é apresentado a gramática da linguagem G-Portugol em notação EBNF.
