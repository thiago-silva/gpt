...
conversões podem ser vistos na tabela \ref{tb_leia_casts}:
...
de operadores. Isso é, não é permitido aplicá-la como um termo em uma expressão
com multiplos termos, embora seja possível utilizá-la em subscritos de vetores e
...
não retornam valor e recebem um parâmetro de um dado tipo (inteiro, real, etc),
que é alimentado com o valor lido. Essa forma exige que a passagem seja feita
por referência. A outra forma, é a implementada em G-Portugol, onde a função não
recebe parâmetros e retorna o valor lido.
...
biblioteca padrão, usada no código C resultante. Da mesma forma, variáveis com o
nome de \textit{leia\_inteiro\_\_} no algoritmo podem causar conflitos da mesma
forma, visto que este é o nome de uma das funçõess incluídas pelo tradutor GPT.
...
T_INT_LIT : T_OCTAL_LIT | T_HEX_LIT | T_BIN_LIT | (T_DIGIT)+

T_OCTAL_LIT : '0' ('c'|'C') [0-8]+

T_HEX_LIT : '0' ('x'|'X') [0-9a-zA-Z]+
