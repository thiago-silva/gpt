...
\title{G-Portugol \normalsize v1.0}
\author{\texttt{thiago.silva@kdemail.net}}
...
lógicamente tem seu valor avaliado como literal. Portanto, são comandos válidos:
...
O ponto fundamental que guia as diretrizes da linguagem é seu propósito educacional: ela deve expressar processos 
...
\subsubsection{A linguagem deve ser totalmente em português respeitando acentuações}

Linguagens de programação reais, em geral, não se utilizam de caracteres especiais (ex. caracteres não pertencentes a 
tabela ASCII) para definição de seu dicionário, visto que são baseadas na língua inglesa. Portanto, letras acentuadas
não são consideradas.

Refletir esse aspecto em G-Portugol evidenciaria as seguintes justificativas hipotéticas:

\begin{itemize}
 \item 
  não acostumar o estudante no uso de termos da linguagem acentuados 
  (já que linguagens de produção não incorporam termos acentuados);

  \item 
  poupar o número de teclas que o estudante tem que digitar ao longo da programação;

  \item
  provável dificuldade em implementar \textit{scanners} que reconheçam letras acentuadas.
\end{itemize}


Esses fatores, e demais não listados, não são suficientes para definir o dicionário de G-Portugol com termos que, 
em portugês, utilizam acentos e caracteres como cedilha. O uso de acentos em termos não cria hábitos ou vícios
que possam compremeter o aprendizado de linguagens reais. Também, o costume de abreviar termos da linguagem servem
para se obter maior produção por parte dos programadores, entretanto G-Portugol não é uma linguagem de produção. E
finalmente, a provável dificuldade em implementar scanners que reconhecem letras acentuadas não é uma justificativa
válida para evitar sua implementação.

A decisão de incorporar palavras que respeitam a lingua portuguesa tem base no ``princípio da menor surpresa''.
A ausência de acentos forçaria o estudante a aprender seus termos excepcionais. Além do mais, o uso de termos 
como ``nao'', chamaria atenção constante do estudante para o fato de a palavra não estar acentuada.

Consequentemente, o uso de acentos permite que a linguagem seja o mais próximo do português quanto for possível,
apoiando a regra de não chamar atenção para si.
...
Seguindo o principio da "menor surpresa" (least surprise), evitar excessões e 
surpresas ao máximo. Isso significa que estruturas e comandos devem ser simples, claros e óbvios. Naturalmente, a 
linguagem portugol já detém uma parcela relevante de características que se acomodam nessas diretrizes, 
no entanto, alguns detalhes ainda merecem atenção. 
 
Deve ser simples, e evitar complexidades que desviam a atenção do propósito original para a detalhes da linguagem. 
(Apenas recursos escenciais devem fazer parte do núcleo.) 

Complementando a última diretriz, essa se refere a recursos disponíveis. 
Para o objetivo em questão, em um primeiro momento, não é interessante equipar a linguagem com recursos como 
alocação dinâmica de memória, threading, etc. De fato, deve ser uma "thin language". Manipulação de memória 
(por meio de variáveis), controles básicos de fluxo, e recursos para entrada e saída simples, devem preencher o 
propósito geral. Esse argumento é baseado na previsão de que a linguagem é utilizada apenas uma porta de 
entrada na área de programação e, portanto, tais recursos devem ser ensinados por meio de linguagens de 
produção, em uma etapa posterior. 
...
ou literatura usada. Pode-se dizer que dialetos de portugol variam de literatura para literatura, e 
assume-se que pode ser interessante permitir que a linguagem se adapte.


  
  *sobre a gramatica
  *sobre a semantica (castings, tipos escolhidos, matrizes, vetores, lack of struct, etc)

\section{Uniformidade estrutural}
